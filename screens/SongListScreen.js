...
class SongListScreen extends Component {
  ...
  componentDidMount() {
    fetch(`http://guohaucong.top:8800/playlist/detail?id=3013629069`)
      .then(res => res.json())
      .then(res => {
  ...
  render() {
    ...
    return (
      <Contsiner>
        <Cover>
          <HeaderTop navigation={this.props.navigation} title={'每日推荐'} />
          <Image
            resizeMode={'stretch'}
            source={{
              uri: CoverImgUrl,
          ...
          <MusicCover>
            <ScrollView>
              {RecommendedScreenData.map((item, index) => (
                <SongListTopChild key={index} data={item} />
              ))}
            </ScrollView>
          </MusicCover>
        </Cover>
...
export default connect(mapStateToProps, mapDispatchProps)(SongListScreen);
